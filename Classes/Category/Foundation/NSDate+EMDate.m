...
- (BOOL)em_isThisYear {
    return [NSDate new].em_year == self.em_year;
}
...
+ (BOOL)em_compareIsOneDate:(NSDate *)date1
                anotherDate:(NSDate *)date2 {
    NSDateFormatter *formatter = [[NSDate new] em_dateFormatWithStr:@"yyyy-MM-dd"];
    NSString *str1 = [formatter stringFromDate:date1];
    NSString *str2 = [formatter stringFromDate:date2];
    return [str1 isEqualToString:str2];
}

+ (NSDate *)em_dateWithIntervel:(NSTimeInterval)interval {
    NSInteger timeInterval = interval;
    if (@(interval).stringValue.length == 13) {
        timeInterval = interval * 0.001;
    }
    
    if (@(timeInterval).stringValue.length == 0) {
        return nil;
    }
